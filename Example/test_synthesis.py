...
i = 0
max_iter = 500 # as in Gatys et al. 2016 implementation
...
for idx_c, content_img in enumerate(content_imgs):
    for idx_s, style_img in enumerate(style_imgs):
        i += 1
        rmse_out = synthesizeImage(style_img, content_img, GramMSELoss, rmse_style_weights, rmse_content_weights, max_iter=max_iter, show_iter=max_iter//3)
        rmse_out.save(f"{output_dir}/style{idx_s}_content{idx_c}_rmse.jpg")
        prs_out = synthesizeImage(style_img, content_img, PearsonCorrelationLoss, prs_style_weights, prs_content_weights, max_iter=max_iter, show_iter=max_iter//3)
        prs_out.save(f"{output_dir}/style{idx_s}_content{idx_c}_prs.jpg")
        cos_out = synthesizeImage(style_img, content_img, CosineSimilarityLoss, cos_style_weights, cos_content_weights, max_iter=max_iter, show_iter=max_iter//3)
