...
from config import max_iter_alt as max_iter, lr_alt as lr # Speed Up 
...
output_dir = 'style_loss_exp/output_alfa'
...
for idx_c, content_img in enumerate(content_imgs):
    for idx_s, style_img in enumerate(style_imgs):
        ...
        sw, cw = style_weights, content_weights
        synthesizeImage(
            vgg, style_img, content_img, GramMSELoss, sw, cw,
            max_iter=max_iter, show_iter=max_iter*3, lr= lr
        ).save(f"{output_dir}/{s_name}_{c_name}_alfa.jpg")
